...
{$ENDREGION}
    procedure Compress(a_m: THashLibUInt32Array);
...
begin
  ...
  begin
    ...
    begin
      Fs_sbox1[i] := ax or (Fsbox[0, b] shl 11);
      Fs_sbox2[i] := bx or (Fsbox[2, b] shl 19);
      Fs_sbox3[i] := cx or (Fsbox[4, b] shl 27);
      Fs_sbox4[i] := dx or (Fsbox[6, b] shl 3);
      System.Inc(i);
      System.Inc(b);
    end;
...
begin
  ...
  THashLibArrayHelper<UInt32>.Clear(THashLibGenericArray<UInt32>(Fm_hash), UInt32(0));

  Inherited Initialize();

end;

procedure TGost.TransformBlock(a_data: THashLibByteArray; a_index: Int32);
var
  m, data: THashLibUInt32Array;
  c, a, b: UInt32;
  i: Int32;
begin
  System.SetLength(m, 8);

  c := 0;

  data := TConverters.ConvertBytesToUInt32(a_data, a_index, 32);
